...
func (d *DevPlugin) Setup(manager plugins.PluginsManager, configurationFolder string) (err error) {
	...
	if d.configuration.MostChosenCount < 1 {
		d.configuration.MostChosenCount = 5
	}
...
func (d *DevPlugin) RunFind(cmd *cobra.Command, args []string) (err error) {
	...
	if cmd.Flags().Changed(flagAdd) {
		return d.addRoot(devAdd)
	}

	if cmd.Flags().Changed(flagDel) {
		return d.deleteRoot(devDel)
	}

	if cmd.Flags().Changed(flagSync) {
		return d.Sync()
	}
...
func (d *DevPlugin) Find(words []string) []string {
	folders := make([]string, 0, DefaultFoldersCount)

	for rootFolder, root := range d.rootsConfiguration.Roots {
		for index := range root.Folders {
			if findpattern.PathContainsPattern(root.Folders[index], rootFolder, words) {
...
func (d *DevPlugin) Sync() (err error) {
	...
	for rootFolder, root := range d.rootsConfiguration.Roots {
		...
		if len(diffs) == 0 {
			...
			colors.Primary("%d changes:\n", len(diffs))

			for index := range diffs {
				if diffs[index][0] == '+' {
					colors.Secondary("%s", diffs[index]+"\n")
				} else {
					colors.Tertiary("%s", diffs[index]+"\n")
				}
			}
		}
	}
...
quickly and get information about it`,
	...
	cmd.Flags().StringVarP(&devAdd, "add", "a", wd, "Add folder to roots")
	cmd.Flags().StringVarP(&devDel, "delete", "d", wd, "Delete folder from roots")
	cmd.Flags().BoolVarP(&devSync, "sync", "s", false, "Sync folders")
